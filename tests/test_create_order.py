import allure
from api.order_api import create_order_auth, create_order_unauth
...
class TestCreateOrder:
    ...
    def test_create_order_with_auth(self, created_user, ingredient_ids):
        ...
        response = create_order_auth(ingredient_ids, token)
    ...
    def test_create_order_without_auth(self, ingredient_ids):
        response = create_order_unauth(ingredient_ids)
        assert response.status_code == 200
        assert "order" in response.json()
        assert "number" in response.json()["order"]
    ...
    def test_create_order_no_ingredients(self, created_user):
        _, token = created_user
        response = create_order_auth([], token)
        assert response.status_code == 400
        assert response.json()["message"] == "Ingredient ids must be provided"

    @allure.title("Создание заказа с невалидным хешем ингредиента возвращает 500")
    def test_create_order_invalid_ingredient(self, created_user):
        _, token = created_user
        # Передаём числа, чтобы получить 500 (невалидный тип данных)
        response = create_order_auth([1, 1323], token)
